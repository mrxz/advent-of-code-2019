...
while(running) {
    ...
    switch(opcode) {
        ...
        case 3:
            ...
            console.log("Moving",  ["", "North", "South", "West", "East"][direction])
            write(modes[2], pc + 1, direction)
            pc += 2;
            break
        case 4:
            if(debug) console.log(pc, modes.join(""), "output", data[pc + 1])
            status = read(modes[2], pc + 1)
            console.log("Status:", status, direction, ["", "North", "South", "West", "East"][direction])
...
const isWall = (pos) => {
    return field[`${pos.x},${pos.y}`] === "#" || !field[`${pos.x},${pos.y}`]
}

// We have "a" path
const aStart = (start, goal, h)  => {
    let openSet = [start]
    let cameFrom = {}
    let gScore = {}
    gScore[start] = h(start, goal)
    
    let fScore = {}
    fScore[start] = h(start, goal)
    
    while(openSet.length) {
        let current = openSet.reduce((best, cur) => (fScore[cur] || Number.MAX_VALUE) < (fScore[best] || Number.MAX_VALUE) ? cur : best)
        if(current === goal) {
            let path = [goal];
        ...
        let curPos = {x: +current.split(",")[0], y: +current.split(",")[1]};
        for(dir of [1,2,3,4]) {
            let neighbourPos = move(curPos, dir)
            let neighbour = `${neighbourPos.x},${neighbourPos.y}`
            if(isWall(neighbourPos)) {
                continue;
            }
            let tentativeGScore = (gScore[current] || Number.MAX_VALUE) + 1
            // console.log("Checking neighbour", neighbourPos, neighbour, tentativeGScore, gScore[current], h(current, goal))
            if(tentativeGScore < (gScore[neighbour] || Number.MAX_VALUE)) {
    ...
    console.log("Failed to find goal:", goal);
}
...
path = aStart("0,0", `${goal.x},${goal.y}`, (pos, goal) => {
    ...
    const goalY = +goal.split(",")[1]
    return (goalX - posX) ** 2 + (goalY - posY) ** 2
})
