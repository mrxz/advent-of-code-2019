...
const execute = async (data, io) => {
    ...
    while(running) {
        ...
        let modes = ("0".repeat(5 - (""+data[pc]).length) + data[pc]).split("").map(x => +x)
        let src1, src2, dest;
        switch(opcode) {
            case 1:
            case 2:
            ...
            case 3:
                ...
                write(modes[2], pc + 1, await io.read())
                pc += 2;
                break
            case 4:
        ...
        return {
            read: () => {
                if(queue[id] && queue[id].length) {
                    console.log("Clearing queue for " + id);
                    do {
                        const p = queue[id].splice(0, 1)[0]
                        input.push(p.x)
                        input.push(p.y)
