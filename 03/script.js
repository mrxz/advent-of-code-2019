...
for(step of b) {
    ...
    while(count--) {
        ...
        signal++
        if(key in field) {
            points.push([x,y,signal + field[key]])
        } else {
            if(!(key in field2)) {
                field2[key] = signal
            }
...
points.reduce((best, c) => {
    if(!best) {
        return c;
    }
    //if((Math.abs(c[0]) + Math.abs(c[1])) < (Math.abs(best[0]) + Math.abs(best[1]))) {
    if(c[2] < best[2]) {
        return c;
    }
    return best;
